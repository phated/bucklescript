...
module Converter = Migrate_parsetree.Convert(Migrate_parsetree.OCaml_404)(Migrate_parsetree.OCaml_402)

let reason_parse lexbuf =
  let ast = Reason_toolchain.RE.implementation lexbuf in
  Jsoo_reactjs_jsx_ppx_v2.process (Converter.copy_structure ast)
